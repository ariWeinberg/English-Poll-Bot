    ...
    with db_session(path) as conn:
        ...
        conn.execute(
            ...
            INSERT OR IGNORE INTO tenants
                (id, name, greenapi_api_url, greenapi_id_instance, greenapi_api_token_instance,
                 gemini_api_key, gemini_model, timezone, summary_enabled, scheduler_enabled,
                 is_active, created_at, updated_at)
            VALUES
                (1, 'Default tenant', 'https://api.green-api.com', '', '',
    ...
    *,
    ...
    if tenant_id is None:
        cursor = conn.execute(
            """
            INSERT INTO tenants (
                name, greenapi_api_url, greenapi_id_instance, greenapi_api_token_instance,
                gemini_api_key, gemini_model, timezone, summary_enabled, scheduler_enabled,
                is_active, created_at, updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    conn.execute(
        """
        UPDATE tenants
        SET name = ?, greenapi_api_url = ?, greenapi_id_instance = ?, greenapi_api_token_instance = ?,
